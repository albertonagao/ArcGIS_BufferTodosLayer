...
def layerOfSelection(inlayer, pontoAnalisado, df, pasta, raio):
    ...
    matchcount = int(arcpy.GetCount_management(inlayer.name)[0]) 
    
    arcpy.AddMessage("Pesquisando:{}      cont:{}".format(a1, matchcount))  
    
    if matchcount == 0:
        arcpy.CreateFeatureclass_management(pasta, a1, geometry_type, inlayer.name)
        a1 = os.path.join(pasta,a1 + ".shp")
    else:
